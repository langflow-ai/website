  ...
  const lines = useMemo<LineCoordinate[]>(() => {
    ...
    if (positions[0]?.width && positions[1]?.width) {
      generatedLines.push({
        id: "line-1",
        start: isMobile
          ? positionHelpers.getBottomRight(positions[0], 50)
          : positionHelpers.getRightCenter(positions[0]),
        end: isMobile
          ? positionHelpers.getTopRight(positions[1], 50)
          : positionHelpers.getTopRight(positions[1], 50),
    ...
    if (positions[1]?.width && positions[2]?.width) {
      generatedLines.push({
        id: "line-2",
        start: isMobile
          ? positionHelpers.getLeftBottom(positions[1], 50)
          : positionHelpers.getLeftCenter(positions[1], 75),
        end: isMobile
          ? positionHelpers.getLeftTop(positions[2], 50)
          : positionHelpers.getTopLeft(positions[2], 25),
        style: isMobile ? "straight" : "curvePath",
      });
    }
    ...
    <div className={`${styles.container} container-wide`} ref={containerRef}>
      <section className={`${styles.heading} ${styles.heading_first}`}>
        <Display
          size={500}
          weight={400}
          className={`${styles.heading_title} ${styles.heading_title_first}`}
        >
          Drag. Drop. Deploy.
        </Display>
        <Display
          size={150}
          weight={300}
          className={`${styles.heading_subtitle} ${styles.heading_subtitle_first}`}
        >
          Don&apos;t let boilerplate code slow you down. Visual state flows,
          reusable components, and rapid iteration for you. Focus on creating AI
          magic.
        </Display>
      </section>
      ...
      <section className={styles.image}>
        ...
        />
        <Image
          src="/mobile-agent.png"
          alt="mobile-agent"
          width={600}
          height={500}
          className={`${styles.responsiveImg} ${styles.mobileImg}`}
        />
      </section>
      ...
      <section className={styles.image_relative}>
        <section className={`${styles.heading} ${styles.heading_absolute}`}>
          ...
          <Display
            size={150}
            weight={300}
            className={`${styles.heading_subtitle} ${styles.heading_subtitle_second}`}
          >
      ...
      <section className={`${styles.heading} ${styles.heading_center}`}>
        ...
        <Display
          size={150}
          weight={300}
          className={`${styles.heading_subtitle} ${styles.heading_subtitle_third}`}
        >
      ...
      <section className={styles.heading}>
        <Display size={400} weight={300} className={styles.heading_title}>Agents at your service</Display>
        <Display
          size={150}
          weight={300}
          className={`${styles.heading_subtitle} ${styles.heading_subtitle_fourth}`}
        >
      ...
      <section className={`${styles.heading} ${styles.heading_last}`}>
        <Display size={400} weight={300} className={styles.heading_title}>Flow as an API</Display>
        <Display
          size={150}
          weight={300}
          className={`${styles.heading_subtitle} ${styles.heading_subtitle_fifth}`}
        >
          Use a free, enterprise-grade cloud to deploy your app
        </Display>
      </section>

      <DrawLine lines={lines} instanceId="dragndrop" />
    </div>
  );
};
